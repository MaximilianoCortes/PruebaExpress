...
async function createUser(req, res) {
  try {
    ...
    const userCreated = await UserModel.create({ name: name, email: email, password: encryptedPassword });
...
async function getUserById(req, res) {
  try {
    ...
    return res.status(500).json({ success: false, message: 'No se pudieron obtener los datos, error de backend.', error: err.message });
  }
}
